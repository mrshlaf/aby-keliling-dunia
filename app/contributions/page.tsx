...
export default function ContributionsPage() {
  ...
  return (
    <div className="relative min-h-screen bg-mesh overflow-hidden pb-48">
      ...
      <div className="container relative z-10 py-10 md:py-24 space-y-12 md:space-y-24 mx-auto px-4 sm:px-6 pb-32">
        
        {/* Massive Wealth Header */}
        <div className="flex flex-col gap-10 lg:flex-row lg:items-end lg:justify-between border-b border-border/40 pb-10 md:pb-16">
          <div className="space-y-4 md:space-y-8 animate-in slide-in-from-bottom duration-1000">
             ...
             <p className="text-muted-foreground text- base md:text-3xl font-medium max-w-sm tracking-tight leading-relaxed">
                Manajemen komitmen finansial untuk ekspedisi Lampung 2026.
             </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 w-full lg:w-auto animate-in scale-in duration-1000">
             <div className="glass-card rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-10 flex flex-col justify-between space-y-6 md:space-y-8 border-white group hover:scale-105 transition-all">
                <div className="h-12 w-12 md:h-14 md:w-14 rounded-xl md:rounded-2xl bg-primary/10 flex items-center justify-center text-primary border border-primary/20">
                   <Wallet size={24} className="md:w-[28px]" />
                </div>
                <div>
                   <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] mb-1 md:mb-2">Total Verified</p>
                   <p className="text-3xl md:text-5xl font-black font-outfit">Rp{totalSaved.toLocaleString('id-ID')}</p>
                </div>
             </div>

             <div className="rounded-[2.5rem] md:rounded-[3rem] bg-foreground text-white p-8 md:p-10 flex flex-col justify-between space-y-6 md:space-y-8 shadow-2xl relative overflow-hidden group hover:scale-105 transition-all">
                <div className="absolute inset-0 bg-gradient-to-br from-primary/20 to-transparent" />
                <div className="h-12 w-12 md:h-14 md:w-14 rounded-xl md:rounded-2xl bg-white/10 flex items-center justify-center backdrop-blur-md border border-white/20 relative z-10">
                   <TrendingUp size={24} className="md:w-[28px]" />
                </div>
                <div className="relative z-10">
                   <p className="text-[10px] font-black opacity-40 uppercase tracking-[0.3em] mb-1 md:mb-2">Quota Remaining</p>
                   <p className="text-3xl md:text-5xl font-black font-outfit">Rp{remaining.toLocaleString('id-ID')}</p>
                </div>
             </div>
          </div>
        </div>
        ...
        <section className={`rounded-[2.5rem] md:rounded-[4rem] p-8 md:p-16 border-4 border-white flex flex-col md:flex-row items-center gap-8 md:gap-16 transition-all shadow-luxury relative overflow-hidden ${
           percentComplete === 100 ? 'bg-primary/5' : 'bg-white/40'
        }`}>
           <div className={`relative h-28 w-28 md:h-48 md:w-48 rounded-full border-[8px] md:border-[16px] flex items-center justify-center text-2xl md:text-5xl font-black font-outfit shadow-2xl transition-all ${
              percentComplete === 100 ? 'text-primary border-primary animate-pulse' : 'text-foreground border-muted'
           }`}>
              <div className="absolute inset-3 md:inset-4 rounded-full border-2 border-dashed border-current opacity-20" />
              {percentComplete}%
           </div>
           
           <div className="flex-1 space-y-3 md:space-y-4 text-center md:text-left">
              <h4 className="text-3xl md:text-6xl font-black font-outfit tracking-tighter leading-none italic">
                 {percentComplete === 100 ? "MISSION ACCOMPLISHED." : "STEADY PROGRESS."}
              </h4>
              <p className="text-muted-foreground font-medium text-base md:text-2xl leading-relaxed tracking-tight max-w-2xl">
                 {percentComplete === 100 
        ...
        <section className="space-y-8 md:space-y-12">
           <div className="flex items-center gap-3 md:gap-4">
              ...
              <h2 className="text-3xl md:text-4xl font-black font-outfit italic tracking-tighter">Commitment <span className="text-primary">Stamps.</span></h2>
           </div>
              ...
              {weeks.map((week, i) => {
                ...
                return (
                  <div 
                    ...
                    className={`glass-card group relative overflow-hidden rounded-[2.5rem] md:rounded-[3.5rem] p-8 md:p-10 transition-all hover:scale-[1.02] hover:shadow-2xl cursor-pointer border-white animate-in slide-in-from-bottom-10 duration-700 ${
                      isDone ? 'opacity-80' : ''
                    }`}
                  ...
                  >
                    <div className="space-y-6 md:space-y-8">
                       <div className="flex items-center justify-between">
                          <div className={`h-16 w-16 md:h-20 md:w-20 rounded-2xl md:rounded-[2rem] flex items-center justify-center transition-all duration-700 ${
                            ...
                            {isDone ? <CheckCircle2 size={32} className="md:w-[40px]" /> : isPending ? <Clock size={32} className="md:w-[40px]" /> : <Plus size={32} className="md:w-[40px]" />}
                          </div>
                          
                          <div className={`px-4 md:px-5 py-1.5 md:py-2 rounded-full text-[8px] md:text-[10px] font-black uppercase tracking-widest border-2 transition-all ${
                       ...
                       <div>
                          <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.4em] mb-1 md:mb-2">Voucher Unit</p>
                          <h3 className="font-black text-4xl md:text-5xl font-outfit tracking-tighter leading-none italic">Week {week.id}</h3>
                          <p className="text-muted-foreground font-black text-[10px] uppercase mt-2 md:mt-3 opacity-60">TARGET: RP{week.target_amount.toLocaleString('id-ID')}</p>
                       </div>

                       <div className="space-y-3 md:space-y-4 pt-6 md:pt-8 border-t border-border/40">
                          <div className="flex justify-between items-end">
                             <p className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.3em]">Current Load</p>
                             <p className="text-2xl md:text-3xl font-black font-outfit text-foreground">
                                Rp{(contribution?.amount || 0).toLocaleString('id-ID')}
                             </p>
                          </div>
                          <div className="h-3 md:h-4 w-full bg-muted rounded-full overflow-hidden p-0.5 md:p-1 border border-border">
                             <div 
          ...
          <div className="w-full max-w-xl bg-white rounded-[4rem] p-12 md:p-16 shadow-[0_0_100px_rgba(0,0,0,0.5)] relative animate-in zoom-in-95 duration-500 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-12">
               ...
               <div className="p-10 rounded-[3rem] bg-muted/10 border-2 border-dashed border-border flex flex-col items-center gap-2">
                ...
                <button 
                   disabled={submitting}
                   className="w-full bg-foreground text-white py-6 md:py-8 rounded-[1.5rem] md:rounded-[2rem] font-black text-[9px] md:text-[10px] uppercase tracking-[0.5em] shadow-3xl shadow-foreground/20 hover:bg-primary transition-all disabled:opacity-50" 
                   onClick={handlePay}
                >
