...
export default function ItineraryPage() {
  ...
  return (
    <div className="container py-8 md:py-16 space-y-12 md:space-y-16 animate-in fade-in duration-700 mx-auto px-4 md:px-6 pb-32">
      
      {/* Dynamic Header */}
      <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between border-b border-border/40 pb-10 md:pb-16">
        <div className="space-y-4 md:space-y-6">
          ...
          <p className="text-muted-foreground text-base md:text-xl font-medium max-w-sm">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-16">
          ...
          <div className="lg:col-span-3 flex lg:flex-col gap-4 overflow-x-auto pb-4 lg:pb-0 scrollbar-hide">
            {days.map((day) => (
              <button 
                ...
                className={`flex items-center gap-4 px-8 py-5 rounded-[2rem] border font-black text-sm uppercase tracking-widest transition-all whitespace-nowrap min-w-[160px] lg:min-w-0 ${
                  activeDay === day.day 
          ...
          <div className="lg:col-span-9 space-y-12">
            {days.find(d => d.day === activeDay)?.activities.map((act: any, idx: number) => (
              <div key={act.id} className="relative pl-12 md:pl-16 group animate-in slide-in-from-bottom-8 duration-700" style={{ animationDelay: `${idx * 150}ms` }}>
                
                {/* Timeline vertical bar */}
                <div className="absolute left-4 md:left-6 top-0 bottom-0 w-[2px] bg-muted/40 group-last:bottom-auto group-last:h-6" />
                
                {/* Timeline dot */}
                <div className={`absolute left-0 top-1.5 h-10 md:h-12 w-10 md:w-12 rounded-2xl flex items-center justify-center shadow-lg transition-all border duration-500 overflow-hidden ${
                ...
                <div className="space-y-6">
                   <div className="space-y-2">
                      <div className="flex items-center gap-3">
                         <span className="text-[10px] md:text-xs font-black text-primary uppercase tracking-[0.2em]">{act.time_range}</span>
                         <div className="h-[1px] w-8 bg-primary/20" />
                         <div className="flex items-center gap-1.5 text-[9px] font-bold text-muted-foreground bg-muted/50 px-2 py-0.5 rounded uppercase">
                            <MapPin size={10} /> {act.location}
                         </div>
                      </div>
                      <h3 className="text-3xl md:text-4xl font-black font-outfit tracking-tight group-hover:text-primary transition-colors">{act.activity}</h3>
                   </div>

                   <div className="p-8 rounded-[2.5rem] bg-white border border-border/40 shadow-luxury group-hover:shadow-2xl transition-all space-y-4">
      ...
      {/* Info Footnote */}
      <section className="mt-16 bg-foreground text-background rounded-[3rem] p-10 md:p-16 relative overflow-hidden group">
         <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full blur-3xl -mr-32 -mt-32" />
         <div className="relative space-y-6">
            <div className="flex items-center gap-3">
               <Info className="text-primary" size={24} />
               <h3 className="text-3xl font-black font-outfit">Catatan Penting.</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-neutral-400 font-medium">
               <p>
